...
$(function () {
    ...
    $('button#btn-next').click(function () {
        ...
        var trustedDomain = '{{ sitemarkers["PlanContractInfo"].url }}';

        // Get the redirection URL
        var redirectUrl = trustedDomain + window.location.search;

        // Perform client-side validation
        if (isValidRedirect(redirectUrl, trustedDomain)) {
            window.location.href = redirectUrl;
        } else {
            // Handle the redirection to an untrusted site
            // For example, redirect to a default or error page
            window.location.href = 'https://tawteenicvdevelopment.powerappsportals.com/'; // Replace with your default page URL
        }
    });
...
function isValidRedirect(redirectUrl, trustedDomain) {
    // Check if the redirectUrl starts with the trustedDomain
    // You can customize this validation logic based on your requirements
    return redirectUrl.startsWith(trustedDomain);
}
